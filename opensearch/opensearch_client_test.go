...
func createTestClient(t *testing.T) *Client {
	...
	addr := os.Getenv("OPENSEARCH_TEST_ADDR")
	if addr == "" {
		t.Skip("set OPENSEARCH_TEST_ADDR to run OpenSearch integration tests")
	}
	...
	if username := os.Getenv("OPENSEARCH_TEST_USERNAME"); username != "" {
		opts = append(opts, WithUsername(username))
	}
	if password := os.Getenv("OPENSEARCH_TEST_PASSWORD"); password != "" {
		opts = append(opts, WithPassword(password))
	}

	cli, err := NewOpenSearchClient(opts...)
	if err != nil {
